								...
								press: $.proxy(function () {
									...
									oVehicleMaster.read("/zc_c_vehicle", {
										...
										success: $.proxy(function (vData) {
											...
											that.getModel("LocalDataModel").setProperty("/VehicleDetails", vData.results[0]);

											var oBusinessModelOnSubmit = that.getModel("ApiBusinessModel");
											oBusinessModelOnSubmit.read("/A_BusinessPartnerAddress", {
												urlParameters: {
													"$filter": "BusinessPartner eq '" + that.getModel("LocalDataModel").getProperty("/VehicleDetails/EndCustomer") +
														"' ",
													"$expand": "to_PhoneNumber,to_FaxNumber,to_EmailAddress,to_MobilePhoneNumber"
												...
												success: $.proxy(function (budata) {
														...
														that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub", budata.results[0]);
														if (budata.results[0].to_EmailAddress.results.length > 0) {
															that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub/EmailAddress", budata.results[0].to_EmailAddress
																.results[
																	0].EmailAddress);
														}
														if (budata.results[0].to_PhoneNumber.results.length > 0) {
															that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub/PhoneNumber", budata.results[0].to_PhoneNumber
																.results[0].PhoneNumber);
														}
														// 		if (budata.results[0].to_FaxNumber.results.length > 0) {
														// 			that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub/FaxNumber", budata.results[0].to_FaxNumber
														// 				.results[0].FaxNumber);
														// 		}
														if (budata.results[0].to_MobilePhoneNumber.results.length > 0) {
											...
											oBusinessModelOnSubmit.read("/A_BusinessPartner", {
												...
												success: $.proxy(function (bpdata) {
													...
													that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub/FirstName", bpdata.results[0].FirstName);
													that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub/LastName", bpdata.results[0].LastName);
													that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub/BusinessPartnerCategory", bpdata.results[
														0].BusinessPartnerCategory);
													...
													if (bpdata.results[0].BusinessPartnerCategory === "1") {
														...
														that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub_Name", bpdata.results[0].FirstName +
															" " + bpdata.results[0].LastName);
														that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub_BpType", that.getView().getModel(
																"i18n")
															.getResourceBundle()
															.getText("Individual")); // added translation

													} else if (bpdata.results[0].BusinessPartnerCategory === "2") {
														that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub_Name", bpdata.results[0].OrganizationBPName1);
														that.getModel("LocalDataModel").setProperty("/VechOwnrSectAddOnAppSub_BpType", this.getView().getModel(
															"i18n").getResourceBundle().getText("Organization"));
													}
